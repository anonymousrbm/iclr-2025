...
from torchrbm.bernoulli_bernoulli.io import (
    load_ais_traj_params,
    load_model,
    load_rcm,
    save_model,
)
from torchrbm.bernoulli_bernoulli.JarJar import (
    compute_ess,
    log_update,
    systematic_resampling,
)
from torchrbm.bernoulli_bernoulli.methods import compute_log_likelihood, create_machine
...
def train(
    ...
    if rcm is not None:
        start_v = sample_rbm(
            rcm["p_m"], rcm["mu"], rcm["U"], args["batch_size"], args["device"], dtype
        )
        start_v = (start_v + 1) / 2
    chains_ll = init_chains(args["batch_size"], params, start_v).to(dtype)
    ...
    parallel_chains.logit_weights = torch.zeros_like(parallel_chains.logit_weights)
    ...
    with torch.no_grad():
        for idx in range(num_updates + 1, args["num_updates"] + 1):
            ...
            # Save the previous parameters of the model
            params_old = params.clone()

            # Save the previous state of the model
            parallel_chains_old = parallel_chains.clone()

            # Sample from the model and compute the gradient
            ...
            # Update the weights of the chains
            dW = log_update(
                chains_old=parallel_chains_old,
                chains_new=parallel_chains,
                params_old=params_old,
                params_new=params,
            )
            parallel_chains.logit_weights += dW
            # print(torch.nn.functional.softmax(-parallel_chains.logit_weights, -1))
            ess = compute_ess(parallel_chains.logit_weights)
            #if ess < args["min_ess"]:
            
            parallel_chains = systematic_resampling(parallel_chains)
            '''if ess < 0.2:
                print("Resampling!",ess.item())
