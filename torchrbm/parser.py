...
def add_args_ptt(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    ptt_args.add_argument(
        ...
        type=str,
        default=None,
        help="(Defaults to None). The path to the RCM file used to initialize the RBM.",
    )
    ptt_args.add_argument(
        "--target_acc_rate",
        type=float,
        default=0.3,
        help="(Defaults to 0.3). The acceptance rate at which to save a new model.",
    )
    return parser

def add_args_JarJar(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ptt_args = parser.add_argument_group("Jarzynski")
    ptt_args.add_argument(
        "--min_ess",
        type=str,
        default=0.5,
        help="(Defaults to 0.5). Minimum effective sample size allowed. It triggers the resampling.",
    )
    return parser
