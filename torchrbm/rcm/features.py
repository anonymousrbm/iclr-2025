...
from torchrbm.custom_fn import unravel_index
...
#     idx_1 = np.random.randint(low=0, high=num_samples - 1, size=num_features)
#     idx_2 = (
#         idx_1 + np.random.randint(low=1, high=num_samples, size=num_features)
#     ) % num_samples
#     dm = dataset[idx_2] - dataset[idx_1]
#     mav = (dataset[idx_1] + dataset[idx_2]) / 2
#     dm /= dm.norm()
#     theta = torch.sum(mav * dm, axis=1)
#     return torch.hstack([dm, theta.unsqueeze(1)])
