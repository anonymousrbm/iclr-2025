...
from torchrbm.rcm.features import evaluate_features
from torchrbm.rcm.log import build_log_string_train
from torchrbm.rcm.proba import compute_p_rcm
from torchrbm.rcm.proba import get_ll_coulomb
...
def compute_pos_grad(
    ...
    """
    grad_vbias_pos = (proj_train * weights).sum(0) / weights.sum()
    # grad_vbias_pos = grad_vbias_pos[int(with_bias) :]
    grad_q_pos = (
...
def compute_neg_grad(
    ...
    # grad_vbias_neg = grad_vbias_neg[int(with_bias) :]
    mu_av = mu.T @ p_m
...
def compute_hessian(
    ...
    """
    device = p_m.device
    # R = torch.hstack([m[:, int(with_bias) :], features_rcm.T])
...
def update_parameters(
    ...
    """
    # intrinsic_dimension = vbias.shape[0] - int(with_bias)
    # prev_vbias_0 = vbias[0:1]
    # params = torch.cat([vbias[int(with_bias) :], q])
    ...
    # if with_bias:
    #     lr = 0.001
    #     new_vbias = mu_av[:1] * lr + prev_vbias_0 * (1.0 - lr)
    #     vbias = torch.concatenate([new_vbias, vbias])
...
def train_rcm(
    ...
    for n_iter in pbar:
        ...
        grad_vbias_neg, grad_q_neg, mu_av = compute_neg_grad(
            m=m, mu=mu, features_rcm=features_rcm, p_m=p_m, with_bias=with_bias
        )
